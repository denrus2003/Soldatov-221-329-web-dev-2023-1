...
searchForm.addEventListener('submit', function(event) {
    ...
    // Делаем запрос на сервер с параметром q
...
searchInput.addEventListener('input', function(event) {
    ...
    // Делаем запрос на сервер для получения вариантов автодополнения
    fetch('http://cat-facts-api.std-900.ist.mospolytech.ru/autocomplete?q=' +
        query)
        .then(response => response.json())
        .then(suggestions => {
            // Очищаем список
            ...
            // Добавляем полученные варианты в список
            suggestions.forEach(suggestion => {
                let listItem = document.createElement('li');
                listItem.textContent = suggestion;
                autocompleteList.appendChild(listItem);

                // Добавляем обработчик событий для элемента списка
